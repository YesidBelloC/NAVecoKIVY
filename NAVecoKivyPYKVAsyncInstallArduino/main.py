...
<CheckParameterArretNS@MDCheckbox>:
    group: 'ParameterArretNS'
    size_hint: None, None
    size: dp(48), dp(48)
                        ...
                        ScrollView:
                            ...
                            MDList:
                                MDTextField:
                                    id: ParamOrdreFiltre
                                    text:"50"
                                    helper_text:"Order Filter Elevation [-]"
...
class MyMDBoxLayoutPieChart(MDBoxLayout):
    grpDistEXT = ObjectProperty(None)
    grpDistINT = ObjectProperty(None)

    grpEnergEXT = ObjectProperty(None)
    grpEnergINT = ObjectProperty(None)

    grpTimeEXT = ObjectProperty(None)
    grpTimeINT = ObjectProperty(None)
...
class NAVecoApp(MDApp):
    ...
    def build(self):
        ...
        self.PredefineLineWidth = 5

        self.Running = False
        self.SpeedState = 0
        self.DistanceState = 0
        self.EnergyState = 0
        ...
        self.ButtonNavigationLayout = MDRelativeLayout(
                MDFillRoundFlatButton(
                                ...
                                text="[b]000[size=40]/000[/size][size=40]\nkm/h[/size][/b]",
                                font_size=220,
                                md_bg_color="#673bb7",
                                # on_release= lambda x: self.ResetRoutInfo(),
                                ),
        ...
        self.PieChartTitles = MDRelativeLayout(
                MDLabel(
                    text="Dist ",
                    halign="center",
        ...
        self.GraphsLayout = MDRelativeLayout(
                MDLabel(
                    text="Dist ",
                    halign="center",
                    pos_hint={"center_x": 0.25, "center_y": 0.9},
    ...
    def on_location(self, **kwargs):
        ...
        if(self.FirstGPSData == False):
            self.root.ids.map.lat = self.LatUsr
            self.root.ids.map.lon = self.LngUsr
            print('Going to Center')
            self.root.ids.map.center_on(self.LatUsr, self.LngUsr)
            self.FirstGPSData = True

            MarketGPSpoint = MapMarker(lat=self.LatUsr,lon= self.LngUsr,source='')
            self.root.ids.map.add_marker(MarketGPSpoint)
            if((MarketGPSpoint.x<self.root.ids.MainScreen.size[0] and MarketGPSpoint.x>0) and (MarketGPSpoint.y<self.root.ids.MainScreen.size[1]*0.85  and MarketGPSpoint.y>0)):

                self.GPSPoint =  Ellipse(angle_start=0,angle_end=360,pos=(MarketGPSpoint.center_x, MarketGPSpoint.y),size=(dp(10),dp(10)))
                if self.root.ids.map.grp2 is not None:
                    # just update the group with updated lines lines
                    self.root.ids.map.grp2.clear()
                    self.root.ids.map.grp2.add(self.GPSPoint)
                else:
                    with self.root.ids.map.canvas.after:
                        ...
                        self.root.ids.map.grp2 = InstructionGroup()
                        self.root.ids.map.grp2.add(self.GPSPoint)
            else:
                self.root.ids.map.grp2.clear()
        ...
        if self.Running==True:
            ...
            self.TimeDelta = time.time()-self.Timekm1
            self.Timekm1 = time.time()
            self.TimeTotal = self.TimeDelta + self.TimeTotal

            # self.times_metre[x]
            # self.vts_metre[x]
            self.SpeedState = self.SpdUsr
            self.DistanceState = self.DistanceState + self.SpdUsr*self.TimeDelta
            self.EnergyState = 0

            CurrentDistance = self.DistanceState
            np_times_metre = np.asarray(self.times_metre)
            idx = (np.abs(np_times_metre - CurrentDistance)).argmin()
            closestDistance = self.times_metre[idx]
            try:
                closestSpeed = self.vts_metre[idx+1]
            except:
                closestSpeed = self.vts_metre[idx]
            # closestDistance = min(self.times_metre, key=lambda x: abs(x-CurrentDistance))
            print("closestSpeed")
            print(closestSpeed)
            print("Speed")
            print(self.SpeedState)
            print("Distance")
            print(self.DistanceState)

            # ext="[b]000[size=40]/000[/size][size=40]\nkm/h[/size][/b]"
            self.ButtonNavigationLayout.ids.RefSpeedNavigation.text = "[b]"+str(int(self.SpdUsr*3.6))+"[size=40]/"+str(int(closestSpeed*3.6))+"[/size][size=40]\nkm/h[/size][/b]"

            self.SpeedDiff = int(np.abs(self.SpdUsr*3.6-closestSpeed*3.6))
            # if self.SpeedDiff>50:
            #     self.SpeedDiff=50

            print("SpeedDiff")
            print(self.SpeedDiff)

            # ff0000 – 00ff00 = FE0100
            # 16711680 – 65280 = 16646400

            # ColorLine = int((self.SpeedDiff*16646400)/50)
            ...
            if self.SpeedDiff>=50:
                ColorLine = hex_to_rgb('ff1800')
            elif self.SpeedDiff<45 and self.SpeedDiff>=40:
                ColorLine = hex_to_rgb('ff6600')
            elif self.SpeedDiff<40 and self.SpeedDiff>=35:
                ColorLine = hex_to_rgb('ffb500')
            elif self.SpeedDiff<35 and self.SpeedDiff>=30:
                ColorLine = hex_to_rgb('ffcc00')
            elif self.SpeedDiff<30 and self.SpeedDiff>=25:
                ColorLine = hex_to_rgb('ffe200')
            elif self.SpeedDiff<25 and self.SpeedDiff>=20:
                ColorLine = hex_to_rgb('faff00')
            elif self.SpeedDiff<20 and self.SpeedDiff>=15:
                ColorLine = hex_to_rgb('d8ff00')
            elif self.SpeedDiff<15 and self.SpeedDiff>=10:
                ColorLine = hex_to_rgb('a0ff00')
            elif self.SpeedDiff<10 and self.SpeedDiff>=5:
                ColorLine = hex_to_rgb('73ff00')
            elif self.SpeedDiff<5 and self.SpeedDiff>0:
                ColorLine = hex_to_rgb('30ff00')
            elif self.SpeedDiff==0:
                ColorLine = hex_to_rgb('35d510')
            ...
            print("red")
            print(ColorLine[0]/255)
            print("green")
            print(ColorLine[1]/255)
            print("blue")
            print(ColorLine[2]/255)

            if self.root.ids.map.grp1 is not None:
                # just update the group with updated lines lines
                ...
                with self.root.ids.map.canvas.after:
                    ...
                    Color(ColorLine[0]/255,ColorLine[1]/255,ColorLine[2]/255,1)
    ...
    def EndingDataReceptionFunct(self):
        ...
        self.root.ids.map.zoom = self.MapZoom
    ...
    def DrawItinerary(self, *args):
        ...
        try:
            ...
            for i in range(len(self.CoordenatesDownSample)):
                Marketn = MapMarker(lat=float(self.CoordenatesDownSample[i][0]),lon= float(self.CoordenatesDownSample[i][1]),source='')
            ...
            lines = Line()
            lines.points = points
            lines.width = self.PredefineLineWidth
    ...
    def ResetRoutInfo(self, *args):
        ...
        try:
            self.root.ids.GraphPieChartID.grpDistEXT.clear()
            self.root.ids.GraphPieChartID.grpDistINT.clear()

            self.root.ids.GraphPieChartID.grpEnergEXT.clear()
            self.root.ids.GraphPieChartID.grpEnergINT.clear()

            self.root.ids.GraphPieChartID.grpTimeEXT.clear()
            self.root.ids.GraphPieChartID.grpTimeINT.clear()

            self.root.ids.GraphSpeedID.remove_widget(graph2)
            self.root.ids.GraphPenteID.remove_widget(graph1)
            self.root.ids.GraphElevID.remove_widget(graph)
            self.root.ids.GraphPieChartID.remove_widget(self.PieChartTitles)
        except:
    ...
    def EndingGeneticAlgorithmFunct(self):
        ...
        # Dist self.root.height,self.root.width

        Data = 20
        DataMax = 100
        DistancePieChartEXT = Ellipse(angle_start=0,angle_end=20,pos=(self.root.width*0.25,self.root.height*0.8),size=(Window.width*0.15,Window.width*0.15))
        DistancePieChartINT = Ellipse(angle_start=0,angle_end=360,pos=(self.root.width*0.25,self.root.height*0.8),size=(Window.width*0.15,Window.width*0.15))

        if self.root.ids.GraphPieChartID.grpDistINT is not None:
            # just update the group with updated lines lines
            self.root.ids.GraphPieChartID.grpDistINT.clear()
            Color(188/255,168/255,231/255,1)
            self.root.ids.GraphPieChartID.grpDistINT.add(DistancePieChartINT)
        else:
            with self.root.ids.GraphPieChartID.canvas.after:
                Color(188/255,168/255,231/255,1)
                self.root.ids.GraphPieChartID.grpDistINT = InstructionGroup()
                self.root.ids.GraphPieChartID.grpDistINT.add(DistancePieChartINT)

        if self.root.ids.GraphPieChartID.grpDistEXT is not None:
            # just update the group with updated lines lines
            self.root.ids.GraphPieChartID.grpDistEXT.clear()
            Color(103/255,59/255,183/255,1)  # line color blue
            self.root.ids.GraphPieChartID.grpDistEXT.add(DistancePieChartEXT)
        else:
            with self.root.ids.GraphPieChartID.canvas.after:
                Color(103/255,59/255,183/255,1)  # line color blue
                self.root.ids.GraphPieChartID.grpDistEXT = InstructionGroup()
                self.root.ids.GraphPieChartID.grpDistEXT.add(DistancePieChartEXT)
        ...
        Data = 20
        DataMax = 100
        EnergiePieChartEXT = Ellipse(angle_start=0,angle_end=20,pos=(self.root.width*0.5,self.root.height*0.8),size=(Window.width*0.15,Window.width*0.15))
        EnergiePieChartINT = Ellipse(angle_start=0,angle_end=360,pos=(self.root.width*0.5,self.root.height*0.8),size=(Window.width*0.15,Window.width*0.15))

        if self.root.ids.GraphPieChartID.grpEnergINT is not None:
            # just update the group with updated lines lines
            self.root.ids.GraphPieChartID.grpEnergINT.clear()
            Color(188/255,168/255,231/255,1)
            self.root.ids.GraphPieChartID.grpEnergINT.add(EnergiePieChartINT)
        else:
            with self.root.ids.GraphPieChartID.canvas.after:
                Color(188/255,168/255,231/255,1)
                self.root.ids.GraphPieChartID.grpEnergINT = InstructionGroup()
                self.root.ids.GraphPieChartID.grpEnergINT.add(EnergiePieChartINT)

        if self.root.ids.GraphPieChartID.grpEnergEXT is not None:
            # just update the group with updated lines lines
            self.root.ids.GraphPieChartID.grpEnergEXT.clear()
            Color(103/255,59/255,183/255,1)  # line color blue
            self.root.ids.GraphPieChartID.grpEnergEXT.add(EnergiePieChartEXT)
        else:
            with self.root.ids.GraphPieChartID.canvas.after:
                Color(103/255,59/255,183/255,1)  # line color blue
                self.root.ids.GraphPieChartID.grpEnergEXT = InstructionGroup()
                self.root.ids.GraphPieChartID.grpEnergEXT.add(EnergiePieChartEXT)
        ...
        Data = 20
        DataMax = 100
        TimePieChartEXT = Ellipse(angle_start=0,angle_end=20,pos=(self.root.width*0.75,self.root.height*0.8),size=(Window.width*0.15,Window.width*0.15))
        TimePieChartINT = Ellipse(angle_start=0,angle_end=360,pos=(self.root.width*0.75,self.root.height*0.8),size=(Window.width*0.15,Window.width*0.15))

        if self.root.ids.GraphPieChartID.grpTimeINT is not None:
            # just update the group with updated lines lines
            self.root.ids.GraphPieChartID.grpTimeINT.clear()
            Color(188/255,168/255,231/255,1)
            self.root.ids.GraphPieChartID.grpTimeINT.add(TimePieChartINT)
        else:
            with self.root.ids.GraphPieChartID.canvas.after:
                Color(188/255,168/255,231/255,1)
                self.root.ids.GraphPieChartID.grpTimeINT = InstructionGroup()
                self.root.ids.GraphPieChartID.grpTimeINT.add(TimePieChartINT)

        if self.root.ids.GraphPieChartID.grpTimeEXT is not None:
            # just update the group with updated lines lines
            self.root.ids.GraphPieChartID.grpTimeEXT.clear()
            Color(103/255,59/255,183/255,1)  # line color blue
            self.root.ids.GraphPieChartID.grpTimeEXT.add(TimePieChartEXT)
        else:
            with self.root.ids.GraphPieChartID.canvas.after:
                Color(103/255,59/255,183/255,1)  # line color blue
                self.root.ids.GraphPieChartID.grpTimeEXT = InstructionGroup()
                self.root.ids.GraphPieChartID.grpTimeEXT.add(TimePieChartEXT)

        self.root.ids.GraphPieChartID.add_widget(self.PieChartTitles)

        # signal = np.array(self.elev_list) (self.Pente)
        ...
        graph = Graph(xlabel='Time [s]', ylabel='Elev [m]', x_ticks_minor=100,
        x_ticks_major=200, y_ticks_major=20,
        y_grid_label=True, x_grid_label=True, padding=5,
        x_grid=True, y_grid=True, xmin=0, xmax=len(self.elev_list), ymin=min(self.elev_list), ymax=max(self.elev_list))
        graph.size_hint=(1, 0.25)
        plot = LinePlot(line_width=4, color=[103/255,59/255,183/255, 1])
        plot.points = [(x, self.elev_list[x]) for x in range(0, len(self.elev_list))]
        graph.add_plot(plot)

        self.root.ids.GraphElevID.add_widget(graph)

        graph1 = Graph(xlabel='Time [s]', ylabel='Slope [rad]', x_ticks_minor=100,
        x_ticks_major=200, y_ticks_major=1,
        y_grid_label=True, x_grid_label=True, padding=5,
        x_grid=True, y_grid=True, xmin=0, xmax=len(self.Pente), ymin=min(self.Pente)*0.9, ymax=max(self.Pente)*1.1)
        graph1.size_hint=(1, 0.25)
        plot = LinePlot(line_width=4, color=[103/255,59/255,183/255, 1])
        plot.points = [(x, self.Pente[x]) for x in range(0, len(self.Pente))]
        graph1.add_plot(plot)

        self.root.ids.GraphPenteID.add_widget(graph1)

        graph2 = Graph(xlabel='Time [s]', ylabel='Speed [m/s]', x_ticks_minor=100,
        x_ticks_major=200, y_ticks_major=30,
        y_grid_label=True, x_grid_label=True, padding=5,
        x_grid=True, y_grid=True, xmin=0, xmax=int(self.times_metre[-1]), ymin=0, ymax=int(max(self.vts_metre_max)*1.1))
        graph2.size_hint=(1, 0.25)
        plot1 = LinePlot(line_width=3, color=[1,0,0,1])
        plot2 = LinePlot(line_width=4, color=[103/255,59/255,183/255, 1])
        plot1.points = [(self.times_metre[x], self.vts_metre_max[x]) for x in range(0, len(self.vts_metre_max))]
        plot2.points = [(self.times_metre[x], self.vts_metre[x]) for x in range(0, len(self.vts_metre))]
        graph2.add_plot(plot1)
        graph2.add_plot(plot2)

        self.root.ids.GraphSpeedID.add_widget(graph2)
